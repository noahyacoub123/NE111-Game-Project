...
def InputPlayerLetter():
    #This function will let the player choose which letter they want to be in the game, then choosing the computers letter based on that -NY
    letter = "" #Initializing the letter variable
    while not (letter == "X" or letter == "O"):
        print("Hello player, do you want to be X or O? ")
        letter = input().upper() #making sure that the letter chosen will be an upper case letter for simplicity -NY
        if letter == "X":
            return ["X", "O"]
        else:
            return ["O", "X"] #Players letter will be shown as the first index and the computer will be the second -NY

def getComputerMove(board, computerLetter): #Verifies that the choice of letter of the player and computer will be opposite -NY
    if computerLetter == "X":
        playerLetter == "O"
    else:
        playerLetter == "X"
